...
自注意力机制解决输入不是单个向量，且输入的向量的数量不确定的情况。比如进行文本输入或进行翻译的时候。

![fully-connected](fc.png)

对于之前的 fc 而言，第一个 saw 和 第二个 saw 没有任何的区别，那输出就必然是一样的。

要让一个 fc 感知到两个 fc 是不同的，就需要给 fc 输入一些上下文信息。

![self-attention](self-attention.png)

初始的向量通过自注意力层后，输出了带上下文的新的向量，再把新的向量输入到全连接层。

那么，这个上下文是如何获得的呢？简单来说会有一个计算器模块去计算不同向量之间的相关性，相关度记作 α，叫做 attention score。
